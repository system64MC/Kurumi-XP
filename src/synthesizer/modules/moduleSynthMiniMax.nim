...
method synthesize*(module: ModuleSynthMiniMax, x: float64, pin: int, moduleList: array[MAX_MODULES, ModuleSynthGeneric], synthInfos: SynthInfos): float64 =
  ...
  let a = if(moduleA != nil): moduleA.synthesize(moduloFix(x, 1.0), module.inputs[0].pinIndex, moduleList, synthInfos) else: 0.0
  let b = if(moduleB != nil): moduleB.synthesize(moduloFix(x, 1.0), module.inputs[1].pinIndex, moduleList, synthInfos) else: 0.0
